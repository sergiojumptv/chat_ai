...
class Chat():
    ...
    def get_conversations(self):
        ...
        self.conversations_text = db.getAllConversationsPromptsTexts(
            self.username)
            ...
            if message['author'] == 'bot':
                if 'uuid' in message:
                    if message["uuid"] == feedback['uuid']:
                        ...
                        db.give_feedback(feedback.get('prompt'), feedback.get(
                            'uuid'), feedback.get('feedback'))
    ...
    def include_in_conversation(self, include=None, result_text=None, uuid_user=None, results=None, prompt='default_prompt', role='user', feedback=""):
        ...
        # obtener el prev_uuid de las conversaciones preparadas
        prev_uuid_text = ""
        if (conversations_text := self.conversations_text.get(prompt)):
            prev_uuid_text = conversations_text[-1].get("uuid")
        # generar el uuid del menssaje de la conersacion preparada
        gen_uuid_text = str(uuid.uuid4())

        # crear diccionario para los mensajes para ser añadidos a la conversacion local
        ...
        if prompt != 'default_prompt':
            # añadir a la conversacion local
            self.conversations[prompt].append(prepared)
            self.conversations_text[prompt].append(prepared_text)

            '''
            Añadir mensajes a las bases de datos (querys y texts)
            el uuid del mensaje para bbdd de text sera el uuid_user
            '''
            db.agregarMensaje(gen_uuid, prev_uuid, include,
                              role, feedback, self.username, prompt)
            db.agregarMensaje(uuid_user if role == "user" else gen_uuid_text,
                              prev_uuid_text,
                              result_text if result_text else include,
                              role, feedback, self.username, prompt,
...
#            self.tokens_used = response['usage']['total_tokens']
    ...
    def newChat(self, msg, gen_uuid, prompt='default_prompt'):
        ...
        if not no_select:
            transformed_result = transform_results.transform(
                self.result, msg, self.conversations_text.get(prompt))
...
def sendmessage():
    ...
    logging.info("user: " + username +
                 " sending message on prompt: "+chat.current_prompt)
